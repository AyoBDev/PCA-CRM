...
- Daily per-agency sweep (`jobs/certReminderCron.js` → `services/certReminderService.sweepCertRemindersForAgency`, 6 AM PT) sends three staged reminders per renewable cert: `reminder_30day` (≤30 & >7 days), `reminder_7day` (≤7 & >0), `expired_final` (≤0). Channels: email (Brevo), in-app Notification (+ socket), push (stub in `reminderChannels/pushChannel.js` until the employee app ships). Reminders are bundled **per employee**: one email + one in-app notification per sweep lists all of that employee's currently-due certs (each labeled with its own stage) via `buildBatchMessage` / `deliverReminderBatch`, while the `CertReminderLog` ledger still records one row per `(cert, versionKey, stage)` for exactly-once dedup.
