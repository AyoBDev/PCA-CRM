...
describe('POST /employees/:id/onboarding/finalize', () => {
  ...
  it('any-rejected with NO existing onboarding token mints a fresh one (changes_requested path)', async () => {
    ...
    const emp = await prisma.employee.create({ data: { name: 'NoTok EE', email: `fin-notok-${Date.now()}-${Math.random().toString(36).slice(2)}@t.co`, onboardingStatus: 'pending_review' } });
