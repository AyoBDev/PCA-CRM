...
// getScheduleView is a public-token endpoint: it resolves the link via the
// owner connection, then enters the link's agency's tenant context (req.db)
// via enterTokenTenant before touching any tenant data. Mock lib/prisma for
// the token lookup, and lib/tenantPrisma/tenantContext so enterTokenTenant
// hands the handler our db double instead of a real tenant Prisma client.
const mockDb = {
...
jest.mock('../src/lib/tenantContext', () => ({
  runWithTenant: jest.fn((ctx, fn) => fn()),
...
beforeEach(() => {
  jest.clearAllMocks();
  mockDb.timesheet.findMany.mockResolvedValue([]);
});

function mockReq(overrides = {}) {
  return { params: { token: 'tok' }, query: { weekStart: '2026-08-10' }, agency: null, ...overrides };
}
...
describe('getScheduleView — Sandata Client ID resolution', () => {
  test('renders the authorization Sandata ID, not the stale value on the shift', async () => {
    prisma.employeeScheduleLink.findUnique.mockResolvedValue({
      token: 'tok', active: true, employeeId: 7, agencyId: 1,
    ...
    mockDb.authorization.findMany.mockResolvedValue([
    ...
    const res = mockRes();
    await controller.getScheduleView(mockReq(), res);
