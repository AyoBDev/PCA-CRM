...
describe('assignRequirements', () => {
  ...
  beforeAll(async () => {
    const e = await prisma.employee.create({ data: { name: 'Assign EE', email: `assign-${Date.now()}@t.co`, agencyId: 1 } });
    employeeId = e.id;
    dtId = (await prisma.documentType.create({ data: { key: `d-${Date.now()}`, label: 'Doc', sortOrder: 1, agencyId: 1 } })).id;
    ctId = (await prisma.certType.create({ data: { key: `c-${Date.now()}`, label: 'Cert', renewalYears: 1, sortOrder: 1, agencyId: 1 } })).id;
    pdId = (await prisma.policyDocument.create({ data: { key: `p-${Date.now()}`, title: 'Policy', sortOrder: 1, agencyId: 1 } })).id;
  });
  ...
  it('creates one requirement per selection and a linked cert slot', async () => {
    const rows = await prisma.$transaction(tx =>
      assignRequirements(tx, employeeId, { documentTypeIds: [dtId], certTypeIds: [ctId], policyDocumentIds: [pdId] }, 1)
    );
