...
async function tokenFor(role) {
  ...
  const user = await prisma.user.create({
    data: { email: `rev-${role}-${unique}@t.co`, passwordHash: await bcrypt.hash('x', 4), name: `${role} u`, role },
  });
  return { header: { Authorization: `Bearer ${jwt.sign({ id: user.id, role, permissionsVersion: user.permissionsVersion ?? 1 }, JWT_SECRET)}` }, userId: user.id };
}
...
describe('GET /api/onboarding/reviews (admin-only)', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.employeeRequirement.create({ data: { employeeId: e.id, kind: 'document', catalogTypeId: dt.id, status: 'submitted', optional: false } });
    await prisma.employeeRequirement.create({ data: { employeeId: e.id, kind: 'certification', catalogTypeId: dt.id, status: 'required', optional: true } });
  });
