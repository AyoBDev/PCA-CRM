...
describe('portal documents', () => {
  it('lists documents for the linked employee', async () => {
    const { header, employeeId } = await employeeAuthHeader();
    const dt = await prisma.documentType.create({ data: { key: `pd-list-${Date.now()}`, label: 'List Doc', sortOrder: 1 } });
    await prisma.employeeDocument.create({
      data: {
        employeeId, documentTypeId: dt.id, storageKey: 'employee-docs/x/y/z.pdf',
        fileName: 'z.pdf', fileType: 'application/pdf', fileSize: 100,
      },
    });
  ...
  it('uploads a document against a requirement scoped to the linked employee, sanitizing the filename', async () => {
    const { header, employeeId } = await employeeAuthHeader();
    const dt = await prisma.documentType.create({ data: { key: `pd-up-${Date.now()}`, label: 'Upload Doc', sortOrder: 1 } });
  ...
  it('refuses to upload against another employee\'s requirement', async () => {
    ...
    const dt = await prisma.documentType.create({ data: { key: `pd-other-${Date.now()}`, label: 'Other Doc', sortOrder: 1 } });
    const requirement = await prisma.employeeRequirement.create({
      data: { employeeId: otherEmployeeId, kind: 'document', catalogTypeId: dt.id, status: 'required' },
    });
  ...
  it('rejects missing file with 400', async () => {
    const { header, employeeId } = await employeeAuthHeader();
    const dt = await prisma.documentType.create({ data: { key: `pd-nofile-${Date.now()}`, label: 'NoFile Doc', sortOrder: 1 } });
    const requirement = await prisma.employeeRequirement.create({
      data: { employeeId, kind: 'document', catalogTypeId: dt.id, status: 'required' },
    });
...
describe('portal policy ack', () => {
  it('acknowledges a policy requirement scoped to the linked employee', async () => {
    const { header, employeeId } = await employeeAuthHeader();
    const policy = await prisma.policyDocument.create({ data: { key: `pol-${Date.now()}`, title: 'Handbook', version: 1 } });
    const requirement = await prisma.employeeRequirement.create({
      data: { employeeId, kind: 'policy', catalogTypeId: policy.id, status: 'required' },
    });
