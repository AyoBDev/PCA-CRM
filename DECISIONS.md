...
## 2026-09-02 — Payroll export in agency Google-Sheet format

**Decision:** Build the payroll export's sheet-shaping logic in-house as a pure formatter module (`payrollExportFormat.js`), rendered with the already-adopted **ExcelJS**. Use ExcelJS's workbook API directly here rather than the shared `lib/xlsxHelper` `rowsToXlsxBuffer`.

**Options considered:**
- **`lib/xlsxHelper.rowsToXlsxBuffer`** — the house helper, already used for every other export. Rejected *for this one export only*: it writes values and column widths but has no per-cell styling API, and this format is defined by fills and fonts (grey header band, green/red authorization banners, green totals, red annotations). Extending the shared helper with a full styling model would grow a deliberately-narrow utility to serve a single caller.
- **Generic spreadsheet-layout / report-builder libraries** — rejected: the layout encodes agency-specific payroll conventions (client blocks, banner placement and coloring against authorized units, annotation vocabulary, service-code ordering) that no general library models. The value is in the conventions, not the rendering.
- **Custom pure model + direct ExcelJS (chosen)** — `buildExportModel(run, authMap)` returns a renderer-agnostic model (rows with values, fills, fonts, number formats); the controller walks it into an ExcelJS worksheet.

**Why:** the format is a faithful reproduction of a sheet the agency builds by hand, so the conventions are the product and deserve to be unit-testable in isolation from any writer — 30 tests cover the palette, widths, block structure, banner coloring, sort order, cell types and annotation rules. Keeping the model renderer-agnostic means the writer stays swappable. ExcelJS needed no evaluation here: the repo had already migrated to it to escape the `xlsx` advisories, and it was verified to reproduce the target workbook's fills, number formats and column widths exactly.

**Note:** this replaced the previous 13-column debug export, whose extra columns (Units Raw, Void?, Overlap, Void/Review Reason) were not part of the agency payroll sheet. `rowsToXlsxBuffer` remains the right default for every other export.
## 2026-09-03 — Demo agency for sales demos

**Feature:** A one-click, resettable demo tenant a salesperson can walk a prospective agency through — populated across clients, caregivers, authorizations, scheduling, timesheets/PCA form, payroll, certifications and audit history.

**Options considered:**
- **Seed/fixture libraries (`faker`, `factory-bot`-style, `@snaplet/seed`).** Signals: `faker` is large and actively maintained, and `@snaplet/seed` generates Prisma-aware data from the schema. Rejected as the *primary* mechanism: the hard part here is not inventing plausible strings — it is the **domain coherence** between records. A demo only looks real if every client's authorization window is open *today*, shifts land in *this* week, timesheet hours reconcile with their entries, and `run.totalPayable` equals the sum of its visits' payable units. Schema-driven generators produce type-valid rows that violate every one of those invariants, so we'd write the same relationship logic anyway and carry a dependency on top. `faker` would only have replaced a ~200-line hand-curated roster we want to be able to read and adjust for demo narrative.
- **Anonymized clone of real agency data.** Rejected on safety grounds: running real PHI through a scrubber to build a sales asset is a HIPAA exposure with no upside over invented data, and any scrubber bug leaks a real client into a prospect-facing screen.
- **Manual click-through setup before each demo.** Rejected: slow, error-prone, and the data goes stale (authorizations expire, shifts fall out of the current week) so the demo silently degrades.
- **Custom catalog + provisioning service (chosen).** `lib/demoData.js` holds a pure, dependency-free catalog where every date is an *offset in days from provisioning time*; `services/demoAgencyService.js` resolves those offsets and writes the graph.

**Choice:** Build in-house — a pure data catalog plus a provisioning service, exposed as `POST /api/platform/demo-agency` behind a superadmin-only, platform-host-only route and a confirm dialog in the `/platform` console.

**Why:** The value is in the *invariants*, not the fake strings, and those invariants are exactly what an off-the-shelf generator can't express. Expressing every date as an offset resolved at seed time means re-running the reset always yields a demo whose authorizations are live, whose schedule is this week's, and whose certifications sit on the reminder curve — the demo cannot go stale, which was the main failure mode of the manual alternative. Keeping the catalog pure and DB-free makes it unit-testable and reviewable on its own (19 tests pin roster shape, service-code validity, the expired/expiring/healthy cert spread, and the freshness invariant).
...
**S3 age filter (resolved in this branch):** the age filter initially read local-disk mtimes, which don't exist in S3 mode — so every object reported an unknown age and fell through to collectable, making the safeguard a no-op exactly where it mattered. `ListObjectsV2` already returns `LastModified` and it was being discarded, so both modules now expose `listObjects()` returning `{key, mtimeMs}`, with `listKeys()` kept as a key-only view so there is one enumeration path. Unknown age fails closed: `partitionByAge()` never treats a null mtime as collectable and reports those files for manual review, since assuming "unknown" means "old" is the assumption that made this unsafe. Verified in a simulated bucket — a 30-day-old object collects, a 1-minute-old upload is protected, and an object with no `LastModified` is skipped; all three would have been deleted before.

## 2026-09-05 — Authorization correction flow + app-wide label associations

**Feature:** Make "correct an authorization" a first-class action, and fix form label/control associations across the app.

**Build vs adopt:** Both were **build** — no library evaluated, because neither is a library-shaped problem. The correction flow is domain logic specific to this app's authorization lifecycle (correction vs renewal vs inactivation). The label work is a mechanical fix to our own JSX; the "adopt" option there is a *linter*, not a runtime dependency — see below.

**Tooling adopted for the label work:** `@babel/parser` + `@babel/traverse`, already present as transitive deps. Hand-editing 228 sites would have been error-prone, and regex codemods proved actively unsafe (a regex pass spliced a `</div>` inside a self-closing `<button/>` in three places). Parsing to an AST and applying *text splices* kept formatting intact while making the edits structurally correct.

**Rejected: `eslint-plugin-jsx-a11y`.** It would catch future regressions but fixes nothing existing, and the project has no ESLint config at all — adopting it means introducing lint infrastructure plus triaging its full rule set on a 200+ violation codebase. Recorded as a follow-up, not a substitute for the fix. The equivalent guard here is a test (`labelAssociations.smoke.test.jsx`) that asserts no unlabelled control survives, which needs no new dependency.

**Two decisions that changed behaviour deliberately:**
1. **A correction preserves the authorization's existing `manualStatus`.** The old correct-in-place path hardcoded `'active'`. Now that correction can edit *any* field, that would mean fixing a typo on an inactive authorization silently reactivates it. Only a create still defaults to `'active'`.
2. **The coverage gap/overlap warning now shows during a correction** (previously suppressed). Dates are editable in this flow, so the warning is relevant; it stays hidden for Inactive.

**IDs use `useId`, not static strings.** The first codemod emitted ids like `id="role"` and produced **31 ids duplicated across files**. Duplicate DOM ids silently break label association wherever a component renders twice, so that pass was discarded and redone with per-instance ids.

**Labels that named something other than one control got real semantics, not `htmlFor`:** radio groups → `<fieldset>`/`<legend>`; button groups → `role="group"` + `aria-labelledby` (PdfToolbar's colour swatches had *no* accessible name and gained `aria-label`/`aria-pressed`); read-only values → `role="note"` with the `<label>` demoted to `<span>`; one `<label>` wrapping only text → `<span>`.

**Verified:** 193 tests across 36 suites, build clean. A build passing is not sufficient here — Vite compiles `fid(...)` where `fid` is undefined and it crashes only at render. An AST audit over every JSX file (undefined `fid` / dangling labels / hook placement / missing imports) **caught 8 real runtime bugs** where a component used `fid` while the helper sat in a different component in the same file; those pages would have crashed on open. Final audit: 0 undefined `fid`, 0 dangling labels, 0 misplaced hooks, 322 pairs. Both new test suites were mutation-tested (breaking one label fails 6 tests; deleting a `fid` helper fails with `ReferenceError`). Layout verified numerically in-browser rather than by eye — toolbar buttons still share one row, `.form-label` computes to the original 13px/500/block, no horizontal overflow.

**Incidental fix:** `LoginPage.test.jsx` was flaky (~1 run in 5). Root cause proven, not guessed: the login card and `LandingPage` both render `<h1>CareOmni</h1>`, so `waitFor(getByText('CareOmni'))` was satisfied by the *pre-resolution* card and never gated on the landing state. Its sibling fetch-failure test was outright vacuous — it passed without the rejection ever being handled. Both now gate on state-unique signals; both mutation-tested.
